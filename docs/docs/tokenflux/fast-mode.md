...
Fast 模式向上游请求更高优先级的算力通道，排队更短、响应更快。现在可以直接在 API Key 上强制开启，客户端不需要做任何改造。

这对 `Codex` 桌面端特别有用——桌面端界面里没有 Fast 开关，以前只能靠插件改写请求，现在把 Key 的策略设成强制开启即可，**不需要安装任何插件**。

::: warning 费用约为常规的 2 倍
Fast 模式走上游的高优先级定价，实际扣费约为常规模式的 **2 倍**。开启前请确认这是你要的。
:::
...
设置对该密钥的**所有请求**生效，不限于 Codex——普通的对话补全、Responses、Anthropic 格式请求都包括在内。
...
- 当前模型不支持优先级通道
- 平台管理员配置了全局的 Fast 拦截或过滤策略

::: tip 管理员策略优先级更高
平台的全局 Fast 策略高于单个密钥的设置。如果管理员配置了过滤或拦截，密钥上的 `强制开启 Fast` 无法绕过。
:::
