...
请求失败时的判断依据是 **HTTP 状态码 + 响应里的 `message`**。

不知道从哪查起，先看 [排障](/docs/troubleshooting)。

## 响应格式

错误体的结构随入口而变，不要按固定字段解析。
...
Gemini 原生入口的 `error.code` 是 HTTP 状态码数字，错误标识只出现在 `message` 里。

::: warning 不要依赖 code 字段
`code` 和 `error.type` 不是所有入口都有，同一个错误在不同入口的字段名也可能不同。**只有 HTTP 状态码和 `message` 文本是稳定的。**
:::
...
| `API key in query parameter is deprecated. Please use Authorization header instead.` | Key 写在了 URL 查询参数里，改用请求头 |
| `Request body too large, limit is ...` | 请求体超过大小上限（该条为 413） |

Claude Code 版本门禁也在这里：
...
| `INVALID_API_KEY` | `Invalid API key` | Key 不存在，或 Key 长度超过 128 字节 |
...
::: tip 过期和额度用尽不是 401
Key 过期返回 **403**，额度用尽返回 **429**。见下方对应小节。
:::
...
::: warning 余额不足是 403，不是 402
整个网关只有批量图片接口会返回 402。余额问题一律是 403，额度用尽是 429。
:::
...
| `GROUP_DISABLED_FOR_USER` | `API Key 所属公开分组已被禁用` | 该分组对你不可用 |
| `GROUP_NOT_ALLOWED` | `API Key 所属专属分组不再允许当前用户使用` | 专属分组授权被撤销 |

上述情况都需要换一个分组重建 Key，见 [创建 API Key](/docs/tokenflux/create-apikey)。

### 分组能力限制

| message | 含义 |
| --- | --- |
| `The current group does not support the requested model "..."` | 模型不属于该分组。后面通常会附上可用模型列表 |
...
| `API Key is not assigned to any group and cannot be used. Please contact the administrator to assign it to a group.` | Key 没绑分组 |
...
| message | 含义 |
| --- | --- |
| `内容审计命中风险规则，请调整输入后重试` | 请求内容被审核拦截，调整输入后重试 |

### 订阅

订阅相关的 403 会返回一段带 `reason=` 的文本，**读 `reason` 就够了**：
...
| message | 含义 |
| --- | --- |
| `Model "..." is not supported by any configured account in this group` | 分组内没有账号支持这个模型 |
...
| code | message | 含义 |
| --- | --- | --- |
| `API_KEY_QUOTA_EXHAUSTED` | `API key 额度已用完` | 这个 Key 单独设的额度用尽 |
...
订阅额度用尽同样返回带 `reason=` 的文本，`reason` 取值为 `DAILY_LIMIT_EXCEEDED`、`WEEKLY_LIMIT_EXCEEDED` 或 `MONTHLY_LIMIT_EXCEEDED`。
...
有 `Retry-After` 时按它指示的秒数等待。没有时用指数退避，不要立刻重试。

## 5xx 服务端与上游

**上游的错误原文不会透传。** 网关按上游返回的状态码换成自己的标准文案，所以 `message` 是 TokenFlux 的措辞，不是模型服务商的。

| 状态码 | message | 触发条件 |
| --- | --- | --- |
| 502 | `Upstream authentication failed, please contact administrator` | 上游返回 401 |
| 502 | `Upstream access forbidden, please contact administrator` | 上游返回 403 |
| 502 | `Upstream service temporarily unavailable` | 上游返回 500 / 502 / 503 / 504 |
| 502 | `Upstream request failed` | 上游返回其他错误 |
| 502 | `All available accounts exhausted` | 重试完所有可用账号仍失败 |
| 503 | `Upstream service overloaded, please retry later` | 上游返回 529（过载） |
...
502 和 503 多数是上游波动，等 1–2 分钟重试或换个模型。持续出现请按 [排障](/docs/troubleshooting#怎么反馈) 反馈。

网关不会主动返回 529，上游的 529 会被转成 503。
