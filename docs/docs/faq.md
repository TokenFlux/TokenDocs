...
如果访问本站或相关服务时一直断开连接，建议先挂梯子后再试。

## 咱们这个站是不是不支持 embedding 模型？

对。

## Pro 号池和 Plus 号池有什么区别？

`Pro` 号池会包含 `Pro` 模型；在服务压力较大时，`Pro` 号池的访问优先级可能更高。

除此之外，二者没有其他区别。

## 为什么模型检测网站显示造假率很高？

很多模型检测网站或榜单并不完全可靠，部分可能存在买榜、样本偏差或检测方式不透明等问题。

此外，从 OpenRouter 等聚合平台调用的模型，也经常被这类检测工具误判为“假模型”。
