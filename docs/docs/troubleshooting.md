...
模型 ID、分组、客户端、API 地址和完整报错是最关键的五项。同一条报错在不同组合下原因可能完全不同，缺少任一项都会显著增加定位成本。

对比信息尤其重要。「换用其他分组正常」「纯对话正常，涉及文件编辑失败」这类描述能显著缩小排查范围。反馈前请确认：更换分组、模型或客户端后问题是否仍然存在。

企业级接入或渠道合作可加 QQ 群 `794504445`，见 [FAQ](/docs/faq)。
