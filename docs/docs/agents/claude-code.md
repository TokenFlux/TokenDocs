...
安装完成后，选择以下两种方式之一将 `Claude Code` 接入 `TokenFlux`。
...
如果你通过 VSCode 或 Zed 启动 `Claude Code`，可以在 `~/.claude/settings.json` 或 `%userprofile%\.claude\settings.json` 的 `env` 中补充：
